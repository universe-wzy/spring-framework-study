...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 *
	 * 如果SAX应用程序需要实现自定义处理外部实体，则必须实现setEntityResolver方法向SAX驱动器注册一个实例
	 */
...
//			return registerBeanDefinitions(doc, resource);  根据返回的 Document 注册 Bean 信息
			// 将定位到的xml文件解析为dom，解析过程由documentLoader完成
			Document doc = doLoadDocument(inputSource, resource);
			// 这里是启动对bean定义的详细过程，该解析过程用到spring的bean配置规则
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			// 委派给了XmlValidationModeDetector来处理
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		// 在实例化BeanDefinitionReader的时候已经初始化了registry，默认是DefaultListableBeanFactory
		// 获得统计前bean的数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 加载及注册bean，具体实现由DefaultBeanDefinitionDocumentReader完成
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// 记录本次加载的BeanDefinition数量
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		// 利用工具类反射实例化指定的对象，这么做的意义好像与为了兼容kotlin而设计的，暂时留个疑问，后续如果涉及到kotlin可以深入一下
		// 这里只需要知道，类似new了一个对象即可
