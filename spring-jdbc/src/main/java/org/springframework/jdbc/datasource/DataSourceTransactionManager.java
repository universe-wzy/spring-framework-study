...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	 ...
	 *
	 * 完善transaction，包括设置CollectionHolder、隔离界别、超时设置，如果是新连接，绑定到当前线程
	 *
	 * 1、尝试获取连接
	 * 		并不是每次都是获得新的连接，如果当前线程的ConnectionHolder依据存在，则没有必要再次获取，
	 * 		或者对于事务同步表示设置为true的需要重新获取连接
	 * 2、设置隔离级别以及只读标识
	 * 		spring针对只读进行了一些操作，但是核心实现是设置connection上的readOnly属性，同样的，隔离级别的控制也是这样的
	 * 3、更改默认的提交设置
	 * 		将提交委托给spring来操作
