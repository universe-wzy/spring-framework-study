...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 3种国际化方式：
	 * 	1、基于url参数的配置
	 * 			locale=zh_CH 对应AcceptHeaderLocalResolver
	 * 	2、基于session
	 * 			会话中解析区域，如果会话属性不存在，根据accept-language HTTP头部信息确定
	 * 			对应 SessionLocaleResolver
	 * 	3、cookie的国际化标准
	 ...
	 *
	 * DispatcherServlet在选用HandlerMapping的过程中，将根据指定的优先级进行排序，优先使用优先级高的，如果当前的
	 * HandlerMapping能够返回可用的handler，则使用当前返回的handler，不再继续访问，否则继续按照优先级访问，直到
	 * 获得一个可用的handler为止
	 *
	 */
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				// 如果是MultipartContent类型的request则转换为MultipartHttpServletRequest类型的request
				...
				if (mappedHandler == null) {
					// 如果没有找到对应的handler则通过response反馈错误信息 万恶的404
				...
				// 根据当前的handler支持last-modified头处理
				// 客户端第一次输入url，服务器返回200，同时在响应头添加last-modified，
				// 第二次请求此url时，发送请求头If-Modified-Since，询问服务器当前请求内容是否被改过，如果服务端内容没有变化，则自动返回304状态码
