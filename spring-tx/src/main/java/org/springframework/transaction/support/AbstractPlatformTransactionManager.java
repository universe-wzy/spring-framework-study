...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	 ...
	 * 3、事务超时设置验证
	 * 4、事务PropagationBehavior属性的设置与验证
	 * 5、构建DefaultTransactionStatus
	 * 6、完善transaction，包括设置CollectionHolder、隔离界别、超时设置，如果是新连接，绑定到当前线程
	 * 7、
	 *
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		// 判断当前线程是否存在事务，依据是当前线程记录的连接不为空，且连接中(connectionHolder)中的transactionActive属性不为空
		...
		//	mandatory	adj. 强制的；托管的；命令 n. 受托者（等于mandatary）
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				...
				// 如果没有可以使用保存点的方式进行回滚，那么嵌入式事务的建立初始建立保存点
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		// 如果在事务链中已经被标识为回滚，那么不会尝试提交事务，直接回滚
	 ...
	 *
	 * 1、自定义触发器的调用，包括回滚前回滚后的调用，
	 * 2、除了触发器监听，就是真正的回滚
	 * 		· 当之前已经保存的事务信息中有保存点信息的时候，使用保存点信息进行回滚。常用于嵌入式事务，对于嵌入式事务的处理，
	 * 		内嵌的事务异常并不会引起外部事务的回滚
	 * 		· 当之前已经保存的事务信息中的事务为新事务，那么直接回滚。常用于单独事务的处理。对于没有保存点的回滚，spring同样是
	 * 		使用底层数据库连接提供的API来操作的。由于我们使用的是DataSourceTransactionManager，那么doRollback函数会使用此类
	 * 		中的实现 doRollBack(DefaultTransactionStatus status)
	 * 		· 当前事务信息中表明是存在事务的，又不属于以上两种情况，多数用于JAT，只做回滚标识，等到提交的时候统一不提交
	 * 3、回滚后的信息清除
	 *
	 *
	 * @param status object representing the transaction
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				// 激活所有的TransactionSynchronization中的方法
				triggerBeforeCompletion(status);
				...
				if (status.hasSavepoint()) {
					...
					if (status.hasTransaction()) {
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							...
							// 如果当前事务不是独立事务，则只能标识状态，等到事务链执行完成后统一回滚
